...
class SnsManager {
    ...
    public func start(type: SnsType) {
        if type == .Kakao {
            ...
            FacebookUtils.getFacebookInfo(from_vc: mViewController!, callback: { userInfo in
                
                self.mSnsUserInfo.user_name = userInfo.name
                gMeInfo.user_name = userInfo.name
                gMeInfo.user_sns_id = String(userInfo.id)
                gMeInfo.user_login_type = .Facebook
                self.requestLogin()
            }) { _ in
                self.delegate?.snsAuthError(type, msg: "페이스북정보를 얻어올수 없습니다.")
            }
        } else if type == .Naver {
